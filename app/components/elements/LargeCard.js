...
export default class LargeCard extends Component {
  ...
  render() {
    ...
    const actionButton = (<ActionButton
      ...
      styleIcon={{
        marginLeft: 0,
        // width: isSmallScreen ? 22 : 36,
        height: isSmallScreen ? 22 : 36
      }}
    />)
    const colorBalance = { color: AppStyle.mainColor }
    const colorBalanceUSD = { color: AppStyle.secondaryTextColor }
    const balanceSecret = isShow ? `${Helper.formatETH(balanceValue)} ETH` : constant.SECRET_WORK
    const balanceUSDSecret = isShow
      ? `$${Helper.formatUSD(balanceUSDValue)}`
      ...
      >
        ...
        >
          <View style={[styles.container, { backgroundColor: backgroundCard }, style]}>
            ...
            />
            <Text style={[styles.balance, colorBalance]}>{balanceSecret}</Text>
            <Text style={[styles.balanceUSD, colorBalanceUSD, { marginBottom: 6 }]}>{balanceUSDSecret}</Text>
            <SyncBalance />
